...
contract wsXMRLiquidityRouter is IwsXmrLiquidityRouter {
    ...
    function isPositionOutOfRange(uint256 tokenId) external view returns (bool) {
        ...
        // Also check via tick for precision
        if (tick <= tickLower || tick >= tickUpper) return true;

        // Double-check via sqrtPrice
        ...
        returns (uint256 daiAmount, uint256 wsxmrAmount)
    {
        ...
        uint256 amount0 = (uint256(liq) * (1 << 96) * (sqrtUpper - sqrtPrice))
            / (sqrtUpper * sqrtPrice);
        uint256 amount1 = (uint256(liq) * (sqrtPrice - sqrtLower)) / (1 << 96);
