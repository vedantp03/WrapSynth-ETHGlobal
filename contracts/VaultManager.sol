...
contract VaultManager is Secp256k1, ReentrancyGuard, Ownable {
    ...
    struct BurnRequest {
        ...
        uint256 wsxmrAmount;
        uint256 xmrAmount;
        uint256 lockedCollateral; // Collateral locked in vault (remains liquidatable)
    ...
    function finalizeMint(bytes32 _requestId, bytes32 _secret) external nonReentrant {
        ...
        // Just mint the tokens to the user
    ...
    function requestBurn(
        ...
        // 2. Check available (unlocked) collateral
        uint256 availableCollateral = vault.collateralAmount - vault.lockedCollateral;
        if (availableCollateral < collateralToLock) revert InsufficientCollateral();
        ...
        vault.lockedCollateral += collateralToLock;
        ...
        burnRequests[requestId] = BurnRequest({
            ...
            xmrAmount: xmrAmount,
            lockedCollateral: collateralToLock, // Locked in vault, remains liquidatable
    ...
    function finalizeBurn(bytes32 _requestId, bytes32 _secret) external nonReentrant {
        ...
        // CRITICAL FIX: UNLOCK collateral back to LP (don't transfer to user!)
        // User already received XMR on Monero chain by revealing this secret
        // LP successfully fulfilled the atomic swap, so they get their collateral back
        // Debt was already reduced in requestBurn
        uint256 collateralToUnlock = request.lockedCollateral;
        vault.lockedCollateral -= collateralToUnlock;
        // collateralAmount stays the same - just unlocking, not removing!
    ...
    function claimSlashedCollateral(bytes32 _requestId) external nonReentrant {
        ...
        // CRITICAL: Transfer locked collateral to user and unlock from vault
        // Debt was already reduced in requestBurn
        uint256 collateralToTransfer = request.lockedCollateral;
        vault.lockedCollateral -= collateralToTransfer;
        vault.collateralAmount -= collateralToTransfer;
        
        // Transfer collateral to user as penalty for LP failure
        if (vault.collateralAsset == address(0)) {
            (bool success, ) = payable(request.user).call{value: collateralToTransfer}("");
            require(success, "ETH transfer failed");
        } else {
            IERC20(vault.collateralAsset).safeTransfer(request.user, collateralToTransfer);
        }
        
        // Clear lockedCollateral for code hygiene (prevents future extension bugs)
        request.lockedCollateral = 0;
        request.status = BurnStatus.SLASHED;
        emit BurnSlashed(_requestId, request.user, collateralToTransfer);
    }
