...
pub struct FinalizeBurn<'info> {
    ...
    #[account(
        ...
        constraint = lp_collateral_account.owner == vault.lp_address
    )]
    ...
    #[account(
        ...
        constraint = user_collateral_account.owner == burn_request.user
    )]
...
pub fn handler(ctx: Context<FinalizeBurn>, secret: [u8; 32]) -> Result<()> {
    ...
    require!(
        ...
        ErrorCode::BurnDeadlineNotReached
    );
    
    let computed_hash = crate::utils::hash_secret(&secret);
    require!(
        computed_hash == burn_request.secret_hash,
    ...
    let transfer_to_lp_ctx = CpiContext::new_with_signer(
    ...
    token::transfer(transfer_to_lp_ctx, burn_request.locked_collateral)?;
    
    if burn_request.reward_collateral > 0 {
        ...
        token::transfer(transfer_reward_ctx, burn_request.reward_collateral)?;
    }
    
    let total_locked = burn_request.locked_collateral
    ...
    burn_request.set_status(BurnStatus::Completed);
    
    msg!("Burn finalized");
    msg!("Collateral released to LP: {}", burn_request.locked_collateral);
    msg!("Reward sent to user: {}", burn_request.reward_collateral);
